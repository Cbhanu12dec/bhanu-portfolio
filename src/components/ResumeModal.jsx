...
export default function ResumeModal({ open, onClose }) {
  ...
  return (
    <div onClick={(e) => { if (e.target === e.currentTarget) onClose() }} style={{
      ...
      <div style={{
        background:'var(--white)', borderRadius: isMobile ? '20px 20px 0 0' : 20,
        ...
        {/* Header */}
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding: isMobile ? '1rem 1.2rem' : '1.2rem 1.6rem', borderBottom:'1px solid var(--blue-100)', flexShrink:0 }}>
          <div style={{ fontFamily:"'Playfair Display', serif", fontSize: isMobile ? '1rem' : '1.2rem', fontWeight:700, color:'var(--text-dark)' }}>📄 Bhanu Cheryala — Resume</div>
          <div style={{ display:'flex', gap:'.6rem', alignItems:'center' }}>
            {!isMobile && (
              <a href="https://github.com/Cbhanu12dec" download style={{ background:'var(--blue-600)', color:'white', padding:'.45rem 1.1rem', borderRadius:8, border:'none', fontSize:'.82rem', fontWeight:600, cursor:'pointer', textDecoration:'none', display:'inline-flex', alignItems:'center', gap:'.4rem' }}>⬇ Download</a>
            )}
            <button onClick={onClose} style={{ width:36, height:36, borderRadius:8, border:'1px solid var(--blue-100)', background:'var(--blue-50)', color:'var(--text-mid)', fontSize:'1rem', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>✕</button>
          </div>
        </div>

        {/* Body */}
        <div style={{ flex:1, overflowY:'auto', padding: isMobile ? '1.2rem' : '2rem', background:'var(--off-white)' }}>
          <div style={{ textAlign:'center', marginBottom:'1.5rem' }}>
            <div style={{ fontFamily:"'Playfair Display', serif", fontSize: isMobile ? '1.6rem' : '2rem', fontWeight:900, color:'var(--text-dark)' }}>Bhanu Cheryala</div>
            <div style={{ fontSize:'.78rem', color:'var(--text-light)', marginTop:'.3rem', lineHeight:1.6 }}>cbhanu12dec@gmail.com · linkedin.com/in/bhanu-cheryala<br />github.com/Cbhanu12dec · Dallas, TX · (838) 910-9856</div>
            <p style={{ fontSize:'.83rem', color:'var(--text-mid)', lineHeight:1.7, marginTop:'.8rem', maxWidth:640, marginInline:'auto' }}>
          ...
          <ResumeSection title="Technical Skills">
            <div style={{ display:'grid', gap:'.5rem', fontSize:'.8rem' }}>
            ...
            {expData.map((job, i) => (
              <div key={i} style={{ marginBottom:'1.3rem' }}>
                <div style={{ display:'flex', justifyContent:'space-between', flexWrap:'wrap', gap:'.3rem' }}>
                  <div>
                    <div style={{ fontWeight:700, fontSize:'.9rem', color:'var(--text-dark)' }}>{job.title}</div>
                    <div style={{ color:'var(--blue-600)', fontSize:'.83rem', fontWeight:600 }}>{job.company}</div>
                  </div>
                  <div style={{ fontSize:'.76rem', color:'var(--text-light)', whiteSpace:'nowrap' }}>{job.date}</div>
                </div>
                <ul style={{ listStyle:'none', marginTop:'.4rem' }}>
                  {job.bullets.map((b, j) => (
                    <li key={j} style={{ position:'relative', paddingLeft:'1rem', fontSize:'.8rem', color:'var(--text-mid)', lineHeight:1.65, marginBottom:'.3rem' }}>
                      <span style={{ position:'absolute', left:0, color:'var(--blue-500)', fontSize:'1.1rem', lineHeight:1.3 }}>·</span>{b}
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <ResumeSection title="Education">
            <div style={{ display:'flex', justifyContent:'space-between', flexWrap:'wrap', gap:'.3rem' }}>
              <div>
                <div style={{ fontWeight:700, fontSize:'.9rem', color:'var(--text-dark)' }}>University at Albany, SUNY</div>
                <div style={{ color:'var(--blue-600)', fontSize:'.83rem', fontWeight:600 }}>M.S. Computer Science · GPA 3.9</div>
              </div>
              <div style={{ fontSize:'.76rem', color:'var(--text-light)' }}>May 2024</div>
            </div>
          </ResumeSection>
          ...
          <ResumeSection title="Projects">
            {[{title:'Job Scraper & Intelligent Job Alert System', desc:'Automated job discovery platform using Python, Selenium, AWS Lambda, SNS & Twilio — reducing manual search effort by 80%.'},
              ...
              <div key={p.title} style={{ marginBottom:'.9rem' }}>
                <div style={{ fontWeight:700, fontSize:'.88rem', color:'var(--text-dark)' }}>{p.title}</div>
                <p style={{ fontSize:'.8rem', color:'var(--text-mid)', marginTop:'.25rem', lineHeight:1.6 }}>{p.desc}</p>
              </div>
            ))}
          </ResumeSection>
        </div>
...
function ResumeSection({ title, children }) {
  return (
    <div style={{ marginBottom:'1.8rem' }}>
      <div style={{ fontSize:'.68rem', fontWeight:700, letterSpacing:2, textTransform:'uppercase', color:'var(--blue-600)', borderBottom:'2px solid var(--blue-100)', paddingBottom:'.5rem', marginBottom:'.9rem' }}>{title}</div>
      {children}
    </div>
  )
}
